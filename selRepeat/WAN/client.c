...
int main (int argc, char *argv[]){
        ...
        socklen_t length;
        unsigned short ACK, WINDOW_SIZE=13;
        unsigned long ts[13]={0};
        char packet[PACKET_SIZE], req[REQ_SIZE], window[13][PAYLOAD_SIZE];
        ...
        while(SEQ<PACKET_COUNT){
                ...
                while((r=recvfrom(sock, packet, PACKET_SIZE, MSG_DONTWAIT , (struct sockaddr *) &name, &length))>0){
                        ...
                        if(i>=0 && i< WINDOW_SIZE){/*avoid duplicated packets from previous window see window_base shift below*/
                                ...
                                /*prepareConsole();*/
                                fprintf(stderr, "out of order packet SEQ: %lu, window_base: %hu ACK: %hu\n", SEQ, window_base, resp.ack);
                        }
                }
                /*-----------------------------------------------*/
...
bool isTicked(unsigned long *ts){
        ...
        static const unsigned long TIMEOUT=40000000;
